...
def ensure_dirs():
    ...
    実験用のMap64配下のimagesディレクトリ、およびサブディレクトリを生成。
    """
    # ベースとなるディレクトリ
    base_dir = "experimental/Map64/images"
...
def delete_pngs(dir: str) -> None:
    ...
    """
    for filename in [
        i for i in os.listdir(f"experimental/Map64/images/{dir}") if i.endswith(".png")
    ]:
        os.remove(os.path.join(f"experimental/Map64/images/{dir}", filename))
...
class Map64:
    ...
    def replot(
        ...
        plt.grid(True)
        plt.savefig("experimental/Map64/images/contour.png")
        plt.close(fig)
    ...
    def extract_map(
        ...
        cv2.imwrite(
            f"experimental/Map64/images/fluo_raw/{cell_id}.png",
            image_fluo,
        )
        ...
        fig.savefig(f"experimental/Map64/images/points_box/{cell_id}.png", dpi=300)
        fig.savefig(f"experimental/Map64/images/points_box.png", dpi=300)
        plt.close(fig)
        plt.clf()
        ...
        fig.savefig(f"experimental/Map64/images/polar/{cell_id}.png", dpi=300)
        plt.close(fig)
        plt.clf()
        ...
        # リサイズ前の画像を保存
        cv2.imwrite(
            f"experimental/Map64/images/map64_raw/{cell_id}.png",
            high_res_image,
        )
        ...
        # 画像を保存
        cv2.imwrite(
            f"experimental/Map64/images/map64/{cell_id}.png",
            high_res_image,
        )
        high_res_image_colormap = cv2.applyColorMap(high_res_image, cv2.COLORMAP_JET)
        cv2.imwrite(
            f"experimental/Map64/images/map64_jet/{cell_id}.png",
            high_res_image_colormap,
        )
        cv2.imwrite(
            f"experimental/Map64/images/map64_jet.png",
            high_res_image_colormap,
        )
        return cls.perform_pca_on_3d_point_cloud_and_save(
            high_res_image,
            f"experimental/Map64/images/pca_2d/{cell_id}.png",
            f"experimental/Map64/images/pca_1d/{cell_id}.png",
        )

    @classmethod
    def combine_images(cls: Map64, out_name: str = "combined_image.png") -> None:
        map64_dir = "experimental/Map64/images/map64"
        points_box_dir = "experimental/Map64/images/points_box"
        pca_2d_dir = "experimental/Map64/images/pca_2d"
        pca_1d_dir = "experimental/Map64/images/pca_1d"
        map64_jet_dir = "experimental/Map64/images/map64_jet"
    ...
    def extract_probability_map(cls, out_name: str) -> np.ndarray:
        ...
        # 画像を保存
        cv2.imwrite(
            f"experimental/Map64/images/probability_map_{out_name}.png",
            probability_map,
        )
        # 画像をviridis heatmapで保存
        probability_map_jet = cv2.applyColorMap(probability_map, cv2.COLORMAP_VIRIDIS)
        cv2.imwrite(
            f"experimental/Map64/images/probability_map_{out_name}_jet.png",
            probability_map_jet,
        )
...
if __name__ == "__main__":
    ensure_dirs()
    for i in os.listdir("experimental/Map64"):
        if i.endswith(".db"):
            with open(f"experimental/Map64/images/{i}_vectors.txt", "w") as f:
