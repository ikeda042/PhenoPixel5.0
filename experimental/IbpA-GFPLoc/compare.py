...
class IbpaGfpLoc:
    ...
    def __init__(
        self, db_path: str = "experimental/IbpA-GFPLoc/sk326gen120min.db"
    ) -> None:
        # 接続するデータベースのパスを引数で指定可能にする
        connection_string = f"sqlite+aiosqlite:///{db_path}?timeout=30"
        self._engine: AsyncEngine = create_async_engine(connection_string, echo=False)
    ...
    async def get_score_distribution(self) -> List[float]:
        """
        データベース内の各細胞について画像を処理し、ピークスコア（総合スコア）を算出してリストで返す関数。
        """
        scores: List[float] = []
        ...
        for cell in tqdm(cells, desc="Processing cells"):
            ...
            if processed_img is not None:
                score = IbpaGfpLoc.quantify_cell_peak(processed_img, cell)
                scores.append(score)
        return scores

    async def main(self) -> None:
        """
        従来の処理：各細胞の画像を処理し、ピークスコア（＝総合スコア）順にraw画像とjet画像を並べ替えて結合画像として出力する。
        """
        ...
        for cell in cells:
            ...
            gray_img: np.ndarray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            h, w = gray_img.shape
            cell_extent: float = max(
                cell.center_x, w - cell.center_x, cell.center_y, h - cell.center_y
            )
