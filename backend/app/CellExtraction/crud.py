...
class SyncChores:
        ...
        for i, img in enumerate(images):
            if num_channels > 1:
                for j in range(num_channels):
                    all_images.append(
                        Image.open(
                            os.path.join("nd2totiff", f"image_{i}_channel_{j}.tif")
                        )
                    )
            else:
                all_images.append(
                    Image.open(os.path.join("nd2totiff", f"image_{i}.tif"))
                )

        all_images[0].save(
            os.path.join(f"{file_name.split(os.sep)[-1].split('.')[0]}.tif"),
            save_all=True,
            ...
            for n, img in enumerate(images):
                if num_channels > 1:
                    for channel in range(num_channels):
                        ...
                        image.save(
                            os.path.join(
                                "nd2totiff", f"image_{n}_channel_{channel}.tif"
                            )
                        )
                else:
                    array = np.array(img)
                    array = SyncChores.process_image(array)
                    image = Image.fromarray(array)
                    image.save(os.path.join("nd2totiff", f"image_{n}.tif"))

            SyncChores.save_images(images, file_name, num_channels)
        SyncChores.cleanup("nd2totiff")
        num_tiff = SyncChores.extract_tiff(
            tiff_path=os.path.join(
                f"./{file_name.split(os.sep)[-1].split('.')[0]}.tif"
            ),
            mode=mode,
        )
        os.remove(os.path.join(f"./{file_name.split(os.sep)[-1].split('.')[0]}.tif"))
        return num_tiff
    ...
    def extract_tiff(
        ...
        for layer in layers.get(mode, []):
            os.makedirs(os.path.join("TempData", layer), exist_ok=True)
        ...
        with Image.open(tiff_path) as tiff:
            ...
            for i in range(num_pages):
                ...
                filename = os.path.join("TempData", layer, f"{img_num}.tif")
                print(filename)
    ...
    def init(
        ...
        init_folders = [os.path.join("TempData", d) for d in init_folders]
        folders = [
            folder
            for folder in os.listdir("TempData")
        ...
        for i in range(num_tiff // set_num):
            try:
                os.mkdir(os.path.join("TempData", "frames", f"tiff_{i}"))
            except Exception as e:
                print(e)
            try:
                os.mkdir(os.path.join("TempData", "frames", f"tiff_{i}", "Cells"))
            except Exception as e:
                print(e)
            try:
                os.mkdir(os.path.join("TempData", "frames", f"tiff_{i}", "Cells", "ph"))
            except Exception as e:
                print(e)
            try:
                os.mkdir("ph_contours")
            except Exception as e:
                print(e)
            ...
            if mode == "triple_layer":
                try:
                    os.mkdir(
                        os.path.join(
                            "TempData", "frames", f"tiff_{i}", "Cells", "fluo2"
                        )
                    )
                except Exception as e:
                    print(e)
                try:
                    os.mkdir(
                        os.path.join(
                            "TempData", "frames", f"tiff_{i}", "Cells", "fluo2_adjusted"
                        )
                    )
                except Exception as e:
                    print(e)
                try:
                    os.mkdir(
                        os.path.join(
                            "TempData", "frames", f"tiff_{i}", "Cells", "fluo2_contour"
                        )
                    )
                except Exception as e:
                    print(e)
        ...
        for k in range(loop_num):
            image_ph = cv2.imread(os.path.join("TempData", "PH", f"{k}.tif"))
            print(num_tiff)
            if mode == "dual_layer" or mode == "triple_layer":
                image_fluo_1 = cv2.imread(os.path.join("TempData", "Fluo1", f"{k}.tif"))
            if mode == "triple_layer":
                image_fluo_2 = cv2.imread(os.path.join("TempData", "Fluo2", f"{k}.tif"))
            ...
            cv2.imwrite(os.path.join("ph_contours", f"{k}.png"), image_ph_copy)
            n = 0
            if mode == "triple_layer":
...
class ExtractionCrudBase:
    ...
    async def process_cell(self, dbname, i, j):
        ...
        async with async_session() as session:
            ...
            img_ph = await self.load_image(
                os.path.join(
                    "TempData", "frames", f"tiff_{i}", "Cells", "ph", f"{j}.png"
                )
            )
            ...
            if self.mode != "single_layer":
                img_fluo1 = await self.load_image(
                    os.path.join(
                        "TempData", "frames", f"tiff_{i}", "Cells", "fluo1", f"{j}.png"
                    )
                )
    ...
    async def main(self):
        chores = SyncChores()
        dbname = (
            os.path.join("databases", f"{self.file_prefix}-uploaded.db")
            if self.mode != "single_layer"
            else os.path.join(
                "databases", f"{self.file_prefix}-single_layer-uploaded.db"
            )
        )
        ...
        for i in range(iter_n[self.mode]):
            cell_path = os.path.join("TempData", "frames", f"tiff_{i}", "Cells", "ph")
    ...
    async def delete_nd2_file(self, filename: str) -> bool:
        ...
        await asyncio.to_thread(os.remove, os.path.join("uploaded_files", filename))
        return True

    async def get_ph_contours(self, frame_num: int) -> StreamingResponse:
        filepath = os.path.join("ph_contours", f"{frame_num}.png")
