...
const MeanEngine: React.FC<ImageFetcherProps> = ({ dbName, label, cellId }) => {
    ...
    useEffect(() => {
        const fetchImageData = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`${url_prefix}/cells/${dbName}/${label}/${cellId}/mean_fluo_intensities`, { responseType: 'blob' });
    ...
    const handleDownloadCsv = async () => {
        try {
            ...
            link.href = url;
            link.setAttribute('download', `${dbName}_mean_fluo_intensities.csv`);
